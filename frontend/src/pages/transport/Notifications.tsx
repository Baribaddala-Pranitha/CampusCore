...
const TransportNotifications = () => {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await api.get(`/api/admin/notifications`.replace("/api",""));
        // fallback to admin notifications list endpoint used earlier
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground">Route and bus related alerts</p>
          </div>
        ...
        <Card className="p-6 shadow-elegant">
          ...
          <div className="space-y-3">
            ...
            {!loading && notifications.map((notification, index) => (
              <div key={index} className={`p-4 rounded-lg border transition-all hover-lift ${notification.unread ? "bg-primary/5 border-primary" : "bg-muted border-transparent"}`}>
