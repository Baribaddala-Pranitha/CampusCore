  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const res = await api.get(`/admin/students/${s._id}/messages`);
        setMessages(res.map((m:any) => ({ from: m.from, subject: m.subject, message: m.body, time: new Date(m.createdAt || Date.now()).toDateString(), unread: m.unread })));
      } catch (e) {
        console.error(e);
      } finally {
        setLoading(false);
    ...
    <DashboardLayout sidebarItems={sidebarItems} userRole="student">
      <div className="space-y-6">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <Card className="p-6 hover-lift">
            <MessageSquare className="h-8 w-8 mb-2 text-success" />
            <h3 className="text-2xl font-bold mb-2">12</h3>
            <p className="text-muted-foreground">Total Conversations</p>
          </Card>
          <Card className="p-6 hover-lift">
            <Bell className="h-8 w-8 mb-2 text-warning" />
        ...
        <Card className="p-6 shadow-elegant">
          <h3 className="font-semibold text-lg mb-6">Recent Announcements</h3>
          <div className="space-y-3">
            <div className="p-4 bg-primary/10 rounded-lg border border-primary/20">
              <div className="flex justify-between items-start mb-2">
                <p className="font-semibold text-primary">School Picnic - December 20th</p>
                <span className="text-xs text-muted-foreground">Today</span>
              </div>
              <p className="text-sm text-muted-foreground">
                Annual school picnic to Wonderland Park. Permission slips must be submitted by Dec 15th.
              </p>
            </div>
            <div className="p-4 bg-muted rounded-lg">
              <div className="flex justify-between items-start mb-2">
                <p className="font-semibold">Winter Break Schedule</p>
                <span className="text-xs text-muted-foreground">Yesterday</span>
              </div>
              <p className="text-sm text-muted-foreground">
                School will be closed from December 24th to January 5th for winter break.
              </p>
            </div>
          </div>
        </Card>
